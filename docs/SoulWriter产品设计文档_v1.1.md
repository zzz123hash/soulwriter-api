...
CREATE TABLE nvwa_souls (
  ...
  -- 动态数据
  activeParams TEXT DEFAULT '{}',     -- 激活参数 {key: {value, baseValue, delta, reason, lastUpdate}}
  relationships TEXT DEFAULT '{}',    -- 关系网络 {targetId: {type, value, history[]}}
...
CREATE TABLE nvwa_events (
  ...
  activatedParams TEXT DEFAULT '{}',  -- 本次激活的参数
...
1. [x] 女娲角色的attributes字段是否有上限？
   - **答案**：不需要预设上限，激活参数系统按需生成
   
2. [x] Token消耗控制？
   - **答案**：分层记忆 + 状态压缩 + 懒加载

3. [x] 多Agent并发？
   - **答案**：共享API，分离Prompt和记忆
...
```javascript
function buildNvwaPrompt(charA, charB, worldVars, recentMemories) {
  // 1. 压缩角色状态
  const charAPrompt = compressCharState(charA);
  const charBPrompt = compressCharState(charB);
  ...
  // 3. 世界状态
  const worldPrompt = `
世界状态：紧张度${worldVars.tension}，混乱度${worldVars.chaos}，进度${worldVars.progress}%
叙事节奏：${getPacingRule(worldVars.tension)}
`;
...
输出严格JSON：
{
  ...
  "activatedParams": {"参数名": {"value": 数字, "delta": 数字, "reason": "原因"}},
...
```javascript
function compressMemories(memories, maxTokens = 500) {
  // 按重要性排序
  ...
  for (const mem of sorted) {
    ...
    result.push({
      id: mem.id,
      summary: mem.content.slice(0, 50) + '...', // 摘要
