...
class SoulWriterLogger {
  constructor() {
    ...
    this.logKeys = {
      ...
      'BOOKS_OK': { 'zh-CN': '成功', 'en-US': 'Success' },
      'BOOKS_ERROR': { 'zh-CN': '错误', 'en-US': 'Error' }
    };
    this.initUI();
  ...
  initUI() {
    ...
    style.textContent = `
      #sw-toolbar{position:fixed;top:0;left:0;right:0;height:48px;background:rgba(22,33,62,0.98);border-bottom:1px solid #2a2a4a;display:flex;justify-content:space-between;align-items:center;padding:0 16px;z-index:1000}
      ...
      #log-panel{position:fixed;top:56px;right:16px;width:420px;max-height:60vh;background:rgba(26,26,46,0.98);border:1px solid #2a2a4a;border-radius:12px;font-family:monospace;font-size:12px;z-index:999;display:none;flex-direction:column;box-shadow:0 8px 32px rgba(0,0,0,0.4)}
  ...
  log(level, key, message, meta) {
    ...
    const logLabel = this.getLogLabel(key) || key;
